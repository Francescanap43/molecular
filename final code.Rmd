...
### I am submitting this as my project for the programming exam

## packages needed

### List of any required packages:
...
![violin plot](final%20violin%20plot.png)
...
![UMAP by clusters](final%20UMAP%20by%20cell%20type.png)
...
library(celldex) library(SingleR) ref \<- celldex::HumanPrimaryCellAtlasData() pred \<- SingleR(test = sce, ref = ref, labels = ref$label.main)
...
![UMAP by real clusters](final%20clusters.png)

### the 2 plots enlight the fact that the clusters are many more than just the Human Primary Cell Atlas Data so there are other clusters that were not considereded in the data set I used, in fact there are many other types of cells beyond the immune system ones.
...
celltype_counts <- celltype_counts[order(-celltype_counts$Freq), ] print(celltype_counts)
...
![top 25 tissues](top%2025%20tissues.png)
